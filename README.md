# Forms Knot
Forms Knot is an [[Knot|Knot]] implementation responsible for forms submissions handling. It supports
...
[[Knot Election Rule|Knot]] for Forms Knot is pattern `form-[a-zA-Z]`.
...
- `data-knotx-action` - this is a name of an [[Forms Adapter|FormsAdapter]] that will be used to handle submitted data.
...
- It communicates with the [[Forms Adapter|FormsAdapter]] on event bus address `test` for processing POST requests to the services
...
Find out more about contract described above in [[Forms Knot|FormsKnot]].
...
Writing custom Forms Adapter requires fulfilling [[Forms Knot|FormsKnot]] contract.
